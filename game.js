...
const levels = [
  {
    ...
    lever: null,
    gate: { x: 915, y: 265, w: 30, h: 55, type: 'next', locked: false },
    enemy: {
      spawnX: 575,
      y: 400,
      w: 34,
  ...
  },
  {
    ...
    gate: { x: 900, y: 445, w: 30, h: 55, type: 'finish', locked: true },
    enemy: {
      spawnX: 690,
      y: 426,
      w: 34,
...
function updatePlayer(dt) {
  ...
  if (level.lever) {
    ...
    if (overlap(player, leverZone) && !level.lever.pulled && isPressed('KeyE')) {
      ...
      setStatus('Lever pulled! The final gate is now open.');
    }
  }

  if (overlap(player, level.gate)) {
    if (level.gate.locked) {
      setStatus('Gate locked. Find and pull the lever (E).');
      player.x = prevX;
      return;
    }
...
function drawGate(gate) {
  ctx.fillStyle = gate.locked ? '#7a6a55' : '#4ac57a';
...
function drawPlayer() {
  ...
  ctx.fillRect(player.x + 4, player.y, player.w - 8, 14);
  // visor/eyes
  ctx.fillStyle = '#10344a';
  const eyeX = player.facing > 0 ? player.x + player.w - 14 : player.x + 6;
  ctx.fillRect(eyeX, player.y + 4, 8, 4);
  // scarf accent
  ctx.fillStyle = '#ff8a5b';
  ctx.fillRect(player.x + (player.facing > 0 ? 2 : player.w - 8), player.y + 16, 6, 8);
}
...
function drawEnemy() {
  ...
  // slime body
  ctx.fillStyle = colors[enemy.state];
  ctx.beginPath();
  ctx.ellipse(enemy.x + enemy.w / 2, enemy.y + enemy.h / 2 + 2, enemy.w / 2, enemy.h / 2, 0, 0, Math.PI * 2);
  ctx.fill();
...
function drawUI(level) {
  ...
  if (level.lever && !level.lever.pulled) {
    ...
    if (nearLever) {
      ...
      ctx.fillText('Press E to pull lever', level.lever.x - 30, level.lever.y - 10);
    }
  }
}
...
loadLevel(0, 'Level 1 started. Reach the first gate.');
requestAnimationFrame(loop);
